...
async function Downloader(readUrl) {
  ...
  let title = await page.$eval(
    "#unzoom > li:nth-child(3) > a > span",
    (el) => el.innerText
  );
  let chap = await page.$eval(
    "#unzoom > li:nth-child(4) > a > span",
    (el) => el.innerText
  );
...
async function getListChapter(id) {
  ...
  const url = `https://hentaihvn.tv/list-showchapter.php?idchapshow=${id}`;
  const browser = await puppeteer.launch({
    headless: "new",
    defaultViewport: null,
    args: ["--no-sandbox"],
  ...
  if (
    isEmty === "Chủ thớt đang upload ảnh cho chương truyện, các bạn chờ tí nhé!"
  ) {
